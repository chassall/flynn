function [fftResults,frequencies] = doFFTTBT(trimmedEEG)
    ...
    fftResults = fftOutput;
    % Compute the average FFT output for this condition
    % fftResults = squeeze(mean(fftOutput,3));

    [~,fftOutputLength,~] = size(fftResults);
    % create a frequency bin
    frequencies = frequencyResolution:frequencyResolution:fftOutputLength*frequencyResolution;
